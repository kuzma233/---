...
class Ui_bookadmin(object):
    def setupUi(self, bookadmin):
        ...
        self.bookinfos.setSelectionBehavior(QtWidgets.QAbstractItemView.SelectRows)#使得bookinfos中鼠标点击操作选中的是一行而不是一格
        self.bookinfos.setEditTriggers(QtWidgets.QAbstractItemView.EditKeyPressed)#使得bookinfos中鼠标双击行的时候不可直接更改
        self.bookinfos.activated.connect(self.displayinfo)

        self.show_all_items()
        self.iteminfo.setSelectionBehavior(QtWidgets.QAbstractItemView.SelectRows)#使得iteminfo中鼠标点击操作选中的是一行而不是一格
        self.iteminfo.setEditTriggers(QtWidgets.QAbstractItemView.EditKeyPressed)#使得iteminfo中鼠标双击行的时候不可直接更改
...
#*****************************************************书籍管理界面功能*******************************************************
        ...
        try:
            ...
            if prize1 != '':
                ...
                if bookid and bookname and author and booktype and prize and press and totalnum and num and abstract_2 and bookshelf:
                    sql = 'insert into books(书号,书名,作者,类型,价格,出版社,摘要,馆藏册数,在馆册数,书架号,被借次数) values ("%s","%s","%s","%s","%.2f","%s","%s","%d","%d","%s","%d")' % (bookid,bookname,author,booktype,prize,press,abstract_2,num,totalnum,bookshelf,0)
                    cursor.execute(sql)
                    conn.commit()
                    self.show_all_books()
                else:
    ...
    def drop_book(self):
        ...
        if s == -1:
            QMessageBox.warning(self, "警告", "没有选中任何书籍！", QMessageBox.Yes)
            return

        bookid = self.bookinfos.item(s,0).text()
        try:
            sql1 = 'delete from books where 书号="%s"'%(bookid)     #事务
            sql2 = 'delete from borrow where 书号="%s"'%(bookid)
            sql3 = 'delete from item where bookid="%s"'%(bookid)
            cursor.execute(sql2)
            cursor.execute(sql3)
            cursor.execute(sql1)
            ...
            conn.commit()  # 事务提交
            QMessageBox.information(self, "成功", "删除成功！", QMessageBox.Yes)
            self.show_all_books()  # 刷新书籍列表  
    ...
    def update_book(self):
        ...
        try:
            ...
            if prize1 != '':
                prize = float(prize1)
                if bookid and bookname and author and booktype and prize and press and totalnum and num and abstract_2 and bookshelf:
                    sql = 'UPDATE books SET 书号="%s",书名="%s",作者="%s",类型="%s",价格="%.2f",出版社="%s",摘要="%s",馆藏册数="%d",在馆册数="%d",书架号="%s",被借次数="%d" WHERE 书号="%s"' % (bookid,bookname,author,booktype,prize,press,abstract_2,num,totalnum,bookshelf,0,bookid)
                    cursor.execute(sql)
                    conn.commit()
                    self.show_all_books()
                else:
...
#******************************************************借阅归还审批功能*******************************************************
    ...
    def pizhun(self):
        ...
        if s == -1:
            QMessageBox.warning(self, "警告", "没有选中任何借还申请！", QMessageBox.Yes)
            return
        ...
        if selecttype == 'borrow':
            ...
            # conn.commit()
            try:
                cursor.callproc('BorrowBook', (bookid, readerid, itemtime))
                conn.commit()
            ...
            sql5 = 'delete from borrow where 书号="%s" and ID="%s"'%(bookid,readerid)
            cursor.execute(sql5)
            conn.commit()
            sql6 = 'delete from item where bookid="%s" and ID="%s"'%(bookid,readerid)
            cursor.execute(sql6)
            conn.commit()
    ...
    def bohui(self):
        ...
        bookid = self.iteminfo.item(s,1).text()
        readerid = self.iteminfo.item(s,0).text()
        sql = 'delete from item where bookid="%s" and ID="%s"'%(bookid,readerid)
        cursor.execute(sql)
        conn.commit()
        self.show_all_items()
