...
class Ui_systemadmin(object):
    ...
    def deletereader(self):
        ...
        if res == QMessageBox.Yes:
            ...
            try:
                sql1 = 'delete from readers where ID="%s"' %id           #事务
                sql2 = 'delete from borrow where ID="%s"' %id     
                sql3 = 'delete from item where ID="%s"' %id
                sql4 = 'delete from loginrecord where ID="%s"' % id
                cursor.execute(sql2)
                cursor.execute(sql3)
                cursor.execute(sql4)
                cursor.execute(sql1)
