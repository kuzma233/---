...
class Ui_Reader(object):
    ...
    def ReturnBook(self):
        ...
        if res0:
            ...
            if res1:
                nowtime = time.strftime("%Y-%m-%d", time.localtime())
                sql='SELECT * FROM item where ID="%s" and time="%s" and type= "return" and bookid="%s"' % (self.loginID,nowtime,bookid)
                res2=cursor.execute(sql)
                if res2:
                    QMessageBox.warning(self, "警告", "请勿重复提交！", QMessageBox.Yes)
                else:
                    sql = 'INSERT INTO item(bookid,ID,time,type) VALUES ("%s","%s","%s","return")' % (bookid, self.loginID, nowtime)
                    cursor.execute(sql)
                    conn.commit()
                    QMessageBox.warning(self, "提示", "提交成功！", QMessageBox.Yes)
            else:
    ...
    def alterinfo(self):
        ...
        if sex:
            try:  
                sql = 'CALL UpdateGender(%s, %s)'
                cursor.execute(sql, (sex, self.loginID))
                conn.commit()
            except pymysql.MySQLError as e:
                QMessageBox.warning(self, "警告", str(e), QMessageBox.Yes)
        if password:
